...
### 17b. Backend Supabase-ის რეგიონში  ·  **ყველაზე დიდი ეფექტი**

გაზომილი 2026-09-12-ს, კატეგორიის გვერდი (`/products?category=phones`):

```
SELECT 1  →  Supabase (eu-central-1, ფრანკფურტი):  130 ms
             ლოკალური Postgres:                       1.7 ms
```

backend თბილისშია, ბაზა ფრანკფურტში. ყოველი SQL მოთხოვნა ევროპაში მიდის.

```
                        მოთხოვნა   Supabase    ლოკალურად
გასწორებამდე                 12     2 250 ms      41 ms
facet-ების გაერთიანების მერე  6     1 240 ms      37 ms
```

**დარჩენილი ~1.2 წამი მანძილია და SQL-ით აღარ სწორდება.** backend-ის იმავე
რეგიონში გაშვება (Railway / Fly / Render — `eu-central-1`) ამას **~50 ms-მდე**
ჩამოიყვანს.
