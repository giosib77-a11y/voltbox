...
## 8. Cart — 🟡 დახურულია ერთი ცნობილი შეზღუდვით (2026-09-12)

### Guest Cart
- [x] guest დამატება ავტორიზაციის გარეშე — კალათა მთლიანად ბრაუზერშია
- [x] quantity `+` მუშაობს — მარაგით შეზღუდული
- [x] quantity `-` მუშაობს — 0-ზე ჩანაწერი იშლება
- [x] remove მუშაობს — „დაბრუნების" საშუალებით
- [x] **cart total სწორია** — გასწორდა, იხ. ქვემოთ
- [x] cart persists reload-ის შემდეგ — `localStorage`, ჰიდრაციის დაცვით
- [x] unavailable product სწორად მუშავდება — backend უარყოფს, ახლა ქართულად

### Logged-in Cart
- [x] logged-in user-ის cart ინახება — იმავე ადგილას, რაც სტუმრისა
- [ ] **cart database-თან სინქრონდება — არა.** ბაზაში cart ცხრილი არ არსებობს
- [x] login-ის შემდეგ cart behavior — უცვლელი რჩება (სტუმრის კალათა არ იკარგება)
- [ ] **logout-ის შემდეგ cart behavior — არაფერი იცვლება.** საერთო კომპიუტერზე
      შემდეგი მომხმარებელი წინას კალათას დაინახავს
- [ ] სხვადასხვა device/session — **არ სინქრონდება** (იგივე მიზეზი)
- [x] cross-tab synchronization — `storage` event-ით, ერთსა და იმავე ბრაუზერში
...
| # | პრობლემა | გასწორება |
|---|---|---|
| 🟡 | **უფასო მიწოდების ზღვარი float-ით ითვლებოდა.** `1.05 × 2 + 49.30 × 3` = ზუსტად 150.00, float-ში 149.99999999999997 → გვერდი 5 ₾-ს ამატებდა, სერვერი არა. **დაპირებული 155, ჩამოჭრილი 150** — ზუსტად მაშინ, როცა „უფასო მიწოდება 150 ₾-დან" მოწმდება | მთელი თეთრები |
| 🟡 | მყიდველი ინგლისურ შეცდომებს ხედავდა: `Invalid email or password`, `Not enough stock` | `CODE_MESSAGES` შევსებულია; მარაგის შეტყობინება ახლა ამბობს, **რამდენი დარჩა** |

### გადამოწმების მტკიცებულება

```
ჯამების შედარება backend-თან — ყოველი ორხაზიანი კალათა, რომელიც ზუსტად
ზღვარზე ჯდება:

  შემოწმებული            195 883
  ძველი კოდი ცდებოდა       2 412
    აქედან რეალისტური      1 832   (ყველა ხაზი ≥ 1.00 ₾)
  გასწორების შემდეგ            0
```

31 ტესტი ჯამებზე, 4 შეცდომის ტექსტებზე — დამტკიცებული ბაგის დაბრუნებით.

### ⚠️ ცნობილი შეზღუდვა: კალათა ბაზაში არ ინახება

ეს არქიტექტურული გადაწყვეტილებაა, არა ხარვეზი — მაგრამ შედეგები რეალურია:

| რა არ მუშაობს | რას ნიშნავს |
|---|---|
| მოწყობილობებს შორის | ტელეფონზე დამატებული კომპიუტერზე არ ჩანს |
| ბრაუზერის გასუფთავება | კალათა იკარგება |
| logout | კალათა რჩება — საერთო კომპიუტერზე შემდეგი მომხმარებელი დაინახავს |

გასაკეთებელი სამუშაო: `carts` ცხრილი, მიგრაცია, sync-ის endpoint-ები და
შერწყმის წესი (სტუმრის კალათა + შენახული) — **ცალკე ამოცანაა.**
